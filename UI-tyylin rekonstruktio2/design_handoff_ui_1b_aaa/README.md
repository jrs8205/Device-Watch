...
## Näkymät
Etusivu on kuvattu alla riviltä riville; kaikki muut sivut (Sovellukset, Laite, Asetukset, Historia,
Latauksesta lähtien, AppDetailSheet, tyhjä tila, esittely) ovat samalla säännöstöllä tiedostossa
`design/Laitevahti 1b kaikki sivut.dc.html`, ja `SettingsSectionCard` säilyttää signatuurinsa
(`titleRes`, `modifier`, `horizontalAlignment`, `content`) — sisältä vaihtuu vain `Card` →
`Column` + 20 dp sisennys + `HorizontalDivider(color = outline)`, joten 20 kutsupaikkaa viidessä
tiedostossa perii muutoksen koskematta niihin.
...
### Korotettu pinta (pohjalevy, dialogi) — oma kontrastiympäristö
Kortti/pohjalevy ei ole sivun pohja, joten vaalealla teemalla nämä eivät riitä sen päällä
(mitattu: outline 2.78:1, error 6.31:1, meripihka 4.17:1). Korotetulle pinnalle omat sävyt:

| Token | Vaalea (korotettu) | Käyttö |
| --- | --- | --- |
| outline | `#7C7C88` | rakenneviiva ja painikkeen reuna pohjalevyn/dialogin päällä |
| error | `#8C1D24` | vanhentunut sovellus, puuttuva lupa |
| statusWarn (meripihka) | `#664200` | kuukausi käyttämättä, widgetiä ei lisätty |

### Kaikki M3-roolit asetetaan — myös ne joita ei "käytetä"
Yksikin oletukselle jäävä rooli vuotaa M3:n perusvioletin läpi (alapalkki jäi violetiksi, koska
`secondaryContainer` ja `surfaceContainer` puuttuivat). Aseta koko lista:

| Rooli | Tumma | Vaalea | Missä näkyy |
| --- | --- | --- | --- |
| primary | `#90CAF9` | `#0A4C87` | aksenttiteksti, ikonit, linkit |
| onPrimary | `#04283F` | `#FFFFFF` | teksti täytetyn aksentin päällä |
| primaryContainer | `#123047` | `#D6E7F7` | korostetut huomautusrivit |
| onPrimaryContainer | `#C8E4FB` | `#06263D` | em. rivien teksti |
| secondaryContainer | `#2196F3` | `#0A4C87` | navin valintapilleri |
| onSecondaryContainer | `#04283F` | `#FFFFFF` | valitun välilehden ikoni |
| background / surface | `#0A0A0C` | `#FAFAFC` | sivun pohja |
| onSurface | `#F2F2F5` | `#101014` | leipäteksti, mono-arvot |
| surfaceVariant | `#17171B` | `#EFEFF4` | kytkimen ura, ikonipohjat |
| onSurfaceVariant | `#A2A2AE` | `#4E4E58` | osioiden otsikot, selitteet |
| surfaceContainer | `#17171B` | `#FFFFFF` | alapalkki, pohjalevy |
| surfaceContainerHigh | `#1E1E24` | `#FFFFFF` | dialogit, aikavalitsin |
| outline | `#6A6A75` | `#8E8E99` | kaistojen rakenneviivat, painikkeen reuna |
| outlineVariant | `#2A2A32` | `#DADAE2` | mittarin ura |
| error | `#F2B8B5` | `#8C1D24` | vanhentunut sovellus, puuttuva lupa |
| statusOk | `#66BB6A` | `#145A1E` | akkurengas, widget-merkki, myönnetty lupa |
| statusWarn | `#EDA100` | `#664200` | kuukausi käyttämättä, widgetiä ei lisätty |

Tilavärit (akkurengas, widget-merkki, lupamerkit) eivät jää `Color(0xFF4CAF50)`-tyyppisiksi
kovakoodauksiksi vaan tulevat tästä paletista. Widget-huomautuksen taustasävy seuraa **oman
merkkinsä** väriä (vihreä merkki → vihreä sävy, keltainen → keltainen) — ei enää punaista taustaa
keltaisen merkin alla.

### Valinta = aksentti täydellä voimalla
Himmeä valintasäiliö oli vain 1.46:1 pohjaa vasten, eli valittua välilehteä ei erottanut
luotettavasti. Navin valintapilleri, valitut `FilterChip`it ja `SegmentedButton` käyttävät nyt
täyttä aksenttia (`secondaryContainer`) ja `onSecondaryContainer`-tekstiä, ja valinnassa on värin
lisäksi toinen vihje: navissa 3 dp yläviiva, siruissa ja segmentissä check-ikoni.
...
## Tiedostot
- `design/Laitevahti 1b kaikki sivut.dc.html` — **kaikki sivut** tässä tyylissä: neljä välilehteä, Historia, Latauksesta lähtien, pohjalevy, tyhjä tila ja esittely + taulukko siitä, mitä 20 `SettingsSectionCard`-kutsupaikkaa vaatii, sekä täysi M3-roolitaulukko.
